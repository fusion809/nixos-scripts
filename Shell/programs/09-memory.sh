...
function ps_gist {
  if [[ -n $2 ]]; then
    ps_mem ${$(pgrep $1 | tr '\n' ,)%,*},$2 > "$1-nixos-psmem.sh" && gist "$1-nixos-psmem.sh" && rm "$1-nixos-psmem.sh"
  elif [[ -n $1 ]]; then
    ps_apl "$1" > "$1-nixos-psmem.sh" && gist "$1-nixos-psmem.sh" && rm "$1-nixos-psmem.sh"
  fi
}
...
function ps_gistv {
    ...
    if [[ -n "$DESC" ]]; then
         ...
         echo "$Q" > "$DESC-nixos-psmem.sh" && gist "$DESC-nixos-psmem.sh" && rm "$DESC-nixos-psmem.sh"

    fi
}

function ps_gistid {
    ps_mem $@ >> nixos-psmem.sh
}
